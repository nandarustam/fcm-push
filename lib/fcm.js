            ...
            var request = https.request(self.fcmOptions, function(res) {
                ...
                function respond() {
                    ...
                    if (data.indexOf('\"multicast_id\":') > -1) {
                        ...
                        error = (parsed_data.failure) ? data.substring(0).trim() : null;
                        id = (parsed_data.success) ? data.substring(0).trim() : null;
                    } else if (data.indexOf('\"message_id\":') > -1) {
                        //handle topics send
                        parsed_data = JSON.parse(data);
                        id = (parsed_data.message_id) ? data.substring(0).trim() : null;
